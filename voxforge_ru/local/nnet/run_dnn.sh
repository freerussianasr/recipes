...
if [ $stage -le 4 ]; then
  ...
  steps/nnet/train_mpe.sh --cmd "$cuda_cmd" --skip-cuda-check $skip_cuda_check --num-iters 1 --acwt $acwt \
    --do-smbr true $data_fmllr/train data/lang $srcdir ${srcdir}_ali ${srcdir}_denlats $dir || exit 1
...
if [ $stage -le 6 ]; then
  # Re-train the DNN by 1 iteration of sMBR 
  steps/nnet/train_mpe.sh --cmd "$cuda_cmd" --skip-cuda-check $skip_cuda_check --num-iters 2 --acwt $acwt \
    --do-smbr true $data_fmllr/train data/lang $srcdir ${srcdir}_ali ${srcdir}_denlats $dir || exit 1
  # Decode (reuse HCLG graph)
  for ITER in 1 2; do
    # Decode with the trigram swbd language model.
